import re
from text.cleaners.english import english_cleaners
...
_curly_pat = r'(.*?)\{(.+?)\}(.*)'
_curly_re = re.compile(_curly_pat)
...
def text_to_sequence_tf(*args, **kwargs):
    return [text_to_sequence(*args, **kwargs)]
...
def _clean_text(text, cleaner_names):
    for name in cleaner_names:
        cleaner = getattr(english_cleaners, name)
        if not cleaner:
